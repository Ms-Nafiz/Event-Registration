...
export default function AdminDonationListPage() {
  ...
  const calculateSummaries = (dList, gList) => {
    ...
    dList.forEach(d => {
      // Only count approved donations for stats? Or all? Let's count all for now, or maybe just approved.
      // Usually summaries are for "collected" money, so 'approved' makes sense. 
      // But for "pending" tracking, we might want to see potential. 
      // Let's stick to ALL for now to see activity, or Approved for financial report.
      // User asked for "donation eshece" (came in), implies approved/collected.
      // Let's use APPROVED for financial stats.
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      const updatedList = donations.map(d => d.id === id ? { ...d, status: 'approved' } : d);
  ...
  return (
    <div className="font-bangla space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">ডোনেশন ড্যাশবোর্ড</h1>
      ...
      {!loading && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
                ...
                <p className="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-1">সর্বোচ্চ ডোনেশন (গ্রুপ)</p>
                {summary.topGroup ? (
                    <div>
                        <h3 className="text-2xl font-bold">{summary.topGroup.name}</h3>
                        {summary.topGroup.description && <p className="text-xs text-indigo-200 mt-0.5">{summary.topGroup.description}</p>}
                        <p className="text-3xl font-bold mt-2">৳ {summary.topGroup.total.toLocaleString()}</p>
                    </div>
                ) : (
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">মোট সংগ্রহ (ফিল্টারকৃত)</p>
                <h3 className="text-3xl font-bold text-gray-800">৳ {totalAmount.toLocaleString()}</h3>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <p className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">ডোনেশন দেয়নি (গ্রুপ)</p>
                <h3 className="text-3xl font-bold text-red-500">{summary.zeroDonationGroups.length}</h3>
                <p className="text-xs text-gray-400 mt-2">টি গ্রুপ এখনো কোনো ডোনেশন দেয়নি</p>
            </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-3 items-center text-sm">
                ...
                <button 
                    onClick={() => {setFilterMonth(''); setFilterGroup('');}} 
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table className="min-w-full divide-y divide-gray-100">
                    ...
                    <tr>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">তারিখ</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">নাম ও গ্রুপ</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">মাস</th>
                        <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">পরিমাণ</th>
                        <th className="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">স্ট্যাটাস</th>
                        <th className="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">অ্যাকশন</th>
                    </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-50">
                    {loading ? (
                        <tr><td colSpan="6" className="text-center py-12 text-sm text-gray-500">লোড হচ্ছে...</td></tr>
                    ) : filteredDonations.length === 0 ? (
                        <tr><td colSpan="6" className="text-center py-12 text-sm text-gray-500">কোনো তথ্য পাওয়া যায়নি।</td></tr>
                    ) : (
                        filteredDonations.map((d) => (
                        <tr key={d.id} className="hover:bg-gray-50/80 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500 font-mono">
                            {d.date?.toDate ? d.date.toDate().toLocaleDateString('bn-BD') : new Date(d.createdAt).toLocaleDateString('bn-BD')}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-bold text-gray-800">{d.userName}</div>
                            <div className="text-xs text-gray-500 mt-1">
                              {(() => {
                                const group = groups.find(g => g.id === d.groupId);
                                return group ? (
                                  <span title={group.description}>
                                    {group.name} {group.description && <span className="text-gray-400">({group.description})</span>}
                                  </span>
                                ) : (
                                  <span className="text-red-400">অজানা গ্রুপ ({d.groupId})</span>
                                );
                              })()}
                            </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-600">{d.month}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-right text-indigo-600">
                            ৳ {d.amount.toLocaleString()}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                            <span className={`px-2.5 py-1 inline-flex text-[10px] leading-4 font-bold rounded-full ${
                                d.status === 'approved' 
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">মাসিক রিপোর্ট</h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">যেসব গ্রুপ ডোনেশন দেয়নি</h3>
                <div className="max-h-60 overflow-y-auto custom-scrollbar pr-2">
                    {summary.zeroDonationGroups.length > 0 ? (
                        <ul className="space-y-2">
                            {summary.zeroDonationGroups.map(g => (
                                <li key={g.id} className="flex items-center gap-2 text-sm text-gray-600 bg-red-50 px-3 py-2 rounded-lg">
                                    <span className="w-2 h-2 rounded-full bg-red-400"></span>
                                    <span>
                                        <span className="font-medium">{g.name}</span>
                                        {g.description && <span className="text-xs text-gray-400 ml-1">({g.description})</span>}
                                    </span>
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-green-600 text-sm font-medium">সবাই ডোনেশন দিয়েছে! 🎉</p>
                    )}
                </div>
            </div>
        </div>
